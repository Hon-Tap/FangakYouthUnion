                ...
                <?php foreach ($announcements as $index => $ann): ?>
                    <article class="bg-white rounded-2xl p-6 shadow-sm hover:shadow-xl transition-all duration-300 border-l-4 border-fyu-primary group flex flex-col h-full"
                        ...
                        <div class="flex items-center text-sm text-gray-500 mb-3">
                            ...
                            <span class="text-xs text-gray-400"><?= !empty($ann['created_at']) ? date('Y', strtotime($ann['created_at'])) : '' ?></span>
                        </div>
                        ...
                        <a href="<?= $baseUrl ?>announcements.php?id=<?= (int)($ann['id'] ?? 0) ?>" class="inline-flex items-center text-fyu-primary font-semibold text-sm hover:underline mt-auto">
                    ...
                    <article class="group relative bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 border border-gray-100"
                        ...
                        <div class="h-48 overflow-hidden relative bg-gray-200">
                            <?php if (!empty($evt['image'])): ?>
                                <img src="<?= $baseUrl ?>images/events/<?= safeText($evt['image']) ?>"
                                     alt="<?= safeText($evt['title'] ?? 'Event image') ?>"
                        ...
                        <div class="p-6">
                            ...
                            <p class="text-gray-600 text-sm mb-5 line-clamp-2">
                                <?= safeHtmlPreview($evt['description'] ?? '') ?>
                            </p>

                            <a href="<?= $baseUrl ?>events.php?id=<?= (int)($evt['id'] ?? 0) ?>" class="w-full inline-flex justify-center items-center py-2.5 rounded-lg border border-fyu-primary text-fyu-primary font-medium hover:bg-fyu-primary hover:text-white transition-all duration-300">
...
<section class="py-20 bg-fyu-dark text-white relative overflow-hidden">
    ...
    <div class="container mx-auto px-6 relative z-10">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            ?>
                <div class="group cursor-pointer" onclick="openProjectModal('<?= safeText($proj['title']) ?>', '<?= safeText($proj['desc']) ?>', '<?= $baseUrl ?>images/<?= safeText($proj['img']) ?>.jpg')"
                    ...
                    <h3 class="text-xl font-bold mb-1 group-hover:text-fyu-light transition"><?= safeText($proj['title']) ?></h3>
                    <p class="text-gray-400 text-sm"><?= safeText($proj['desc']) ?></p>
                </div>
            <?php endforeach; ?>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const onScroll = () => {
        if (!hasAnimated && counterSection && window.scrollY + window.innerHeight > counterSection.offsetTop + 100) {
