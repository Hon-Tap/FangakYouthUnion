<?php
// sections/members.php (Refactored with Pagination, Smart Images, Responsive UI & Optimized Logic)
// ======================================================================
...
// ======================================================================
// 4. STATS (Overall Database) - OPTIMIZED INTO A SINGLE QUERY
// ======================================================================
$stats_sql = "SELECT 
    COUNT(id) as total,
...
$total_members = $stats['total'] ?? 0;
$active_members = $stats['active'] ?? 0;
$pending_members = $stats['pending'] ?? 0;
$male_members = $stats['male'] ?? 0;
$female_members = $stats['female'] ?? 0;
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-8">
        ...
        <div class="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 flex items-center justify-between transition-transform hover:-translate-y-1">
            <div>
                ...
                <p class="text-sm mt-1 text-slate-600">Male: <span class="font-bold text-blue-600"><?= number_format((float)$male_members) ?></span></p>
                <p class="text-sm text-slate-600">Female: <span class="font-bold text-pink-600"><?= number_format((float)$female_members) ?></span></p>
            </div>
    ...
    <div class="bg-white p-4 md:p-5 rounded-2xl shadow-sm border border-slate-100 mb-6">
        <form method="GET" class="flex flex-col md:flex-row gap-3">
            ...
            <div class="flex gap-2">
                ...
                <a href="<?= htmlspecialchars($base_url) ?>" class="bg-white border border-slate-200 text-slate-600 font-medium rounded-xl px-6 py-2.5 hover:bg-slate-50 transition w-full md:w-auto text-center flex items-center justify-center">Reset</a>
            </div>
        </form>
    </div>
    ...
    <div class="bg-white shadow-sm border border-slate-100 rounded-2xl overflow-hidden flex flex-col">
        <div class="overflow-x-auto overflow-y-auto max-h-[65vh] custom-scrollbar">
            <table class="w-full text-left border-collapse min-w-[800px]">
                <thead class="bg-slate-50 border-b border-slate-200 text-slate-500 text-sm uppercase tracking-wider sticky top-0 z-10 shadow-sm">
                    <tr>
                        <th class="p-4 font-semibold whitespace-nowrap bg-slate-50">
                            <a href="<?= build_sort_link('id', $sort, $dir) ?>" class="hover:text-green-600 flex items-center gap-1">
                                ID <?= $sort === 'id' ? ($dir === 'ASC' ? '↑' : '↓') : '' ?>
                            </a>
                        </th>
                        <th class="p-4 font-semibold whitespace-nowrap bg-slate-50">User</th>
                        <th class="p-4 font-semibold whitespace-nowrap hidden sm:table-cell bg-slate-50">Contact Info</th>
                        <th class="p-4 font-semibold whitespace-nowrap hidden md:table-cell bg-slate-50">Location</th>
                        <th class="p-4 font-semibold whitespace-nowrap text-center bg-slate-50">Status</th>
                        <th class="p-4 font-semibold whitespace-nowrap text-right bg-slate-50">Actions</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100 text-sm">
                    ...
                    <?php foreach ($members as $m): ?>
                        <tr class="hover:bg-slate-50/80 transition-colors group">
                            <td class="p-4 text-slate-500 font-medium whitespace-nowrap">#<?= htmlspecialchars($m['id']) ?></td>
                                ...
                                <div class="flex items-center gap-4">
                                    ...
                                    <?php else: ?>
                                        <div class="h-10 w-10 sm:h-12 sm:w-12 rounded-full bg-slate-100 text-slate-400 flex items-center justify-center font-bold border-2 border-white shadow-sm shrink-0">
                                            <?= htmlspecialchars(strtoupper(substr($m['full_name'], 0, 1))) ?>
                                        </div>
                                    <?php endif; ?>
                                    
                                    <div>
                                        <p class="font-bold text-slate-800"><?= htmlspecialchars($m['full_name']) ?></p>
                                        <p class="text-xs text-slate-500 mt-0.5"><?= htmlspecialchars($m['gender']) ?> • <?= htmlspecialchars($m['age']) ?> yrs</p>
                            ...
                            <td class="p-4 text-right whitespace-nowrap">
                                <?php if ($m['status'] === 'pending'): ?>
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="<?= htmlspecialchars($base_url) ?>&p=<?= $page ?>&action=approve&member_id=<?= $m['id'] ?>" class="p-2 bg-green-50 text-green-600 hover:bg-green-600 hover:text-white rounded-lg transition-colors" title="Approve">
                                        ...
                                        <a href="<?= htmlspecialchars($base_url) ?>&p=<?= $page ?>&action=reject&member_id=<?= $m['id'] ?>" class="p-2 bg-red-50 text-red-600 hover:bg-red-600 hover:text-white rounded-lg transition-colors" title="Reject">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
                                        </a>
                                    </div>
                                <?php else: ?>
                                    <div class="flex items-center justify-end gap-3 text-sm">
                                        <button class="text-blue-600 font-semibold hover:text-blue-800 transition-colors" onclick="openMemberModal('edit', <?= htmlspecialchars($m['id']) ?>)">Edit</button>
                                        <span class="text-slate-300">|</span>
                                        <a href="admin/members_delete.php?id=<?= htmlspecialchars($m['id']) ?>" class="text-rose-500 font-semibold hover:text-rose-700 transition-colors" onclick="return confirm('Delete this member completely?')">Delete</a>
                                    </div>
                                <?php endif; ?>
                            </td>
                        </tr>
        ...
        <div class="border-t border-slate-200 p-4 bg-slate-50 flex items-center justify-between z-10 relative">
